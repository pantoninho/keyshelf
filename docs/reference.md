...
| `keyshelf ls` (alias `keyshelf list`)                                                           | Print an offline project map: every shelf (sorted), its schema's key count, and its environments (sorted) as a tree. A pure file read — builds no provider, contacts no backend, prints no key values (ADR-0008). `--json` returns `{ environments: [{ shelf, stage, keys }] }`.                           |
