...
async function createFixture() {
  ...
  await writeFile(
    ...
    ["keys:", "  db:", "    host: localhost", "    port: 5432", '    password: !secret ""'].join(
      "\n"
    )
  );
...
describe("loadConfig", () => {
  ...
  it("loads schema, env, and app mapping", async () => {
    ...
    expect(config.schema.map((k) => k.path)).toEqual(["db/host", "db/port", "db/password"]);
  ...
  it("loads mapping from custom mappingFile path", async () => {
    ...
    expect(config.appMapping).toEqual([{ envVar: "DB_HOST", keyPath: "db/host" }]);
  });
  ...
  it("custom mappingFile takes precedence over default .env.keyshelf", async () => {
    ...
    expect(config.appMapping).toEqual([{ envVar: "ONLY_HOST", keyPath: "db/host" }]);
  });

  it("throws for missing custom mappingFile with correct path", async () => {
    const missingPath = join(root, "does-not-exist");
    await expect(loadConfig(appDir, "production", { mappingFile: missingPath })).rejects.toThrow(
      missingPath
    );
  });
