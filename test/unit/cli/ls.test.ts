...
describe('describeSource', () => {
  it('returns plaintext override value', () => {
    ...
    expect(describeSource(configKey('db/host', 'localhost'), env)).toBe(
      'override: prod-db',
    );
  });
  ...
  it('returns schema default for config with no override', () => {
    ...
    expect(describeSource(configKey('db/port', '5432'), env)).toBe(
      'default: 5432',
    );
  });
  ...
  it('returns (optional, no value) for optional key with no source', () => {
    const env: EnvConfig = { overrides: {} };
    expect(describeSource(secretKey('auth/token', true), env)).toBe(
      '(optional, no value)',
    );
  });
